...
astexpr Operator(enum exprtype type, int opid, astexpr left, astexpr right)
{
  ...
  n->optimal_size = ComputeExprSize(left, right);
  return (n);
}
...
astspec Specifier(enum spectype type, int subtype, symbol name, astspec body, int size, int sizeexp)
{
  ...
  d->subtype = subtype;
  d->is_spec_for_sng = 0; 
  d->size    = (size == 0) ? 32 : size;
  d->sizeexp = sizeexp;
  d->optimal_size    = size;
  d->optimal_sizeexp = sizeexp;
  d->name    = name;
  d->body    = body;
  d->u.next  = NULL;
                         ...
                         aststmt init, astexpr cond, astexpr incr, aststmt body, astexpr bound)
{
  ...
  n->u.iteration.cond = cond;
  n->u.iteration.incr = incr;
  n->u.iteration.bound = bound;
  return (n);
}

aststmt Iterationstatementbounded(int subtype, aststmt init, astexpr cond, 
                         astexpr incr, astexpr bound, int parallel, aststmt body)
{
  aststmt n = Statement(BOUNDED_ITERATION, subtype, body);
  n->u.bounded_iteration.init = init;   /* Maybe declaration or expression */
  n->u.bounded_iteration.cond = cond;
  n->u.bounded_iteration.incr = incr;
  n->u.bounded_iteration.bound = bound;
  n->u.bounded_iteration.parallel = parallel;
  return (n);
}
...
char *clausenames[20] = { NULL, "nowait", "if", "num_threads", "ordered",
...
char *clausesubs[19] =  { "static", "dynamic", "guided", "runtime",
    "shared", "none", "+", "*", "-", "&", "|",
    "^", "&&", "||", "affinity", "auto",
    "min", "max"           /* OpenMP 3.1 */
};
char *ompdirnames[19] = { NULL, "parallel", "for", "sections", "section",
...
// add the newest condition for the nested priv if to the top of the if condition stack
...
// pop the top element on the if condition stack
