...
var_table      vartab;       /* Variable table for optimal variable sizing */
...
int main(int argc, char *argv[])
{
    ...
    if(argc != 5){
        ...
        if(argc < 5){
          ...
          for(int i = 5; i<argc; i++){
            if (strcmp(argv[i], "-d") == 0){
                ...
                /* Optimal Variable Sizing pass on */
                opt_var_size_pass = 1;
            }
            else{
        ...
        if(paramsERR){
            fprintf(stderr,"Incorrect input parameters:\n");
            fprintf(stderr,"Usage: picco <user program> <SMC config> <translated program> <utility config> <options>\n");
            fprintf(stderr, "Valid options: \n -d (print debug messages)\n -vs (optimal variable sizing pass activated)\n");
            exit(1);
        }
    }
    else{
      /* Debug messages off */
      debug_msg = 0;
      /* Optimal Variable Sizing pass off */
      opt_var_size_pass = 0;
    }
  ...
  ; 
	...
	p = BlockList(p,verbit("#include \"smc-compute/LocationTracking.h\""));
   ...
   // }
    ...
    append_new_main();
    // Print out the ast file
    if(debug_msg)
        ast_stmt_print_file(ast);
    if (opt_var_size_pass)
    {
        vartab = new_var_table();
        var_size_compute(ast, vartab);
        if(debug_msg)
            var_table_print(vartab);
        var_table_clear(vartab);
    }
