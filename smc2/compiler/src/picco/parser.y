...
int     contain_priv_if_flag = 0; /* Flag for if a statement contains a private conditioned if/else */
int     cond_index = 0; 
int     num_index = 0;
...
/* %type <stmt>   iteration_statement_bounded_for */
...
/* Added DSL syntax tokens */
/* Bounded for loops: foreach (intended as parallelizable) and bfor (standard for loop with bound) */
%token <name> BOUND /*FOREACH IN RANGE */
...
cast_expression:
	...
	$$->thread_id = thread_id; 
        ...
        $$->size = $1->size;
	   $$->sizeexp  = $1->sizeexp; 
	   $$->optimal_size = $1->optimal_size;
	   $$->optimal_sizeexp  = $1->optimal_sizeexp;
 	...
 	if($2->spec->subtype == SPEC_int || $2->spec->subtype == SPEC_Rlist && ($2->spec->body->subtype == SPEC_private && $2->spec->u.next->subtype == SPEC_int))
	{
		...
		$$->optimal_size = $2->spec->subtype == SPEC_Rlist ? $2->spec->u.next->optimal_size : $2->spec->optimal_size; 
		tmp_index--;
		$$->index = tmp_index; 
		$$->flag = PRI; 
		//FL2INT
 	...
 	if($2->spec->subtype == SPEC_float || $2->spec->subtype == SPEC_Rlist && ($2->spec->body->subtype == SPEC_private && $2->spec->u.next->subtype == SPEC_float))
	{
		...
		$$->optimal_size = $2->spec->subtype == SPEC_Rlist ? $2->spec->u.next->optimal_size : $2->spec->optimal_size;  
		$$->optimal_sizeexp = $2->spec->subtype == SPEC_Rlist ? $2->spec->u.next->optimal_sizeexp : $2->spec->optimal_sizeexp;
		tmp_float_index--;
...
iteration_statement:
    ...
    | BOUND primary_expression FOR '(' expression ';' expression ';' expression ')' statement
    {
      security_check_for_condition($2);
      security_check_for_condition($7);         
      $$ = BFor(Expression($5), $7, $9, $11, $2);
      $$->flag = $11->flag;
    }
...
*/
/*
iteration_statement_bounded_for:
    BOUND CONSTANT FOR '(' expression ';' expression ';' expression ')' statement
    {
      security_check_for_condition($7);         
      $$ = BFor(Expression($5), $7, $9, $11, Constant(strdup($2));
      $$->flag = $11->flag;
    }
    BOUND CONSTANT FOREACH IDENTIFIER IN RANGE '(' constant_expression ',' constant_expression ')' statement
    {
        security_check_for_condition($8);
        security_check_for_condition($10); 
        $$ = BForE(Expression(Assignment(Symbol($4), ASS_eq, $8)),       
                BinaryOperator(BOP_lt, $4, $10),   
                Assignment(Symbol($4), ASS_add, 1),      
                Constant(strdup($2)),           
                1,                                 
                $12);                            
        $$->flag = $12->flag;
...
void set_identifier_attributes(symbol id, astexpr expr, int is_su_field)
{
   ...
   if(s->subtype == SPEC_int){
		...
		expr->size = s->size;  
		expr->optimal_size = s->optimal_size;
   }
   if(s->subtype == SPEC_float){
		...
		expr->optimal_size = s->optimal_size;
		expr->optimal_sizeexp = s->optimal_sizeexp;
   }
   if(s->subtype == SPEC_struct || s->subtype == SPEC_union)
		id->type = 2; 
      ...
      if(s->body->subtype == SPEC_private){
         if(s->u.next->subtype == SPEC_int){
            id->type = 0;
			   ...
			   expr->optimal_size = s->u.next->optimal_size; 
         }
            ...
            expr->optimal_size = s->u.next->optimal_size;
			   expr->optimal_sizeexp = s->u.next->optimal_sizeexp; 
		   }
      }
      else{
         if(s->u.next->subtype == SPEC_int){
...
void set_bitlength_expr(astexpr e, astexpr e1, astexpr e2)
{
	if(e2 == NULL)
	{
        //printf("setting bitlen: %d\n", e1->size);
		e->size = e1->size; 
		e->sizeexp = e1->sizeexp; 
        e->optimal_size = e1->optimal_size;
        e->optimal_sizeexp = e1->optimal_sizeexp;		
	}
	else 
	{
        // printf("setting bitlen: %d , %d\n", e1->size, e2->size);
		...
		if(e1->optimal_size >= e2->optimal_size){
			e->optimal_size = e1->optimal_size;
		}
		else{
			e->optimal_size = e2->optimal_size;
		}
		if(e1->ftype == 1){	
         ...
         if(e1->optimal_sizeexp >= e2->optimal_sizeexp){
				e->optimal_sizeexp = e1->optimal_sizeexp;
			}
			else{
            e->optimal_sizeexp = e2->optimal_sizeexp;
         }
		}
	}	
}
...
void set_size_symbol(astexpr e1, astexpr e2, astexpr e3){
    ...
    if(set_security_flag_spec(spec) == PUB){
      e1->size = -1; 
      e1->sizeexp = -1; 
	   e1->optimal_size = spec->u.next->optimal_size;
	   e1->optimal_sizeexp  = spec->u.next->optimal_sizeexp;
    }
    else if(spec->type == SPECLIST){
      ...
      e1->size = spec->size;
	   ...
	   e1->optimal_size = spec->optimal_size;
	   e1->optimal_sizeexp  = spec->optimal_sizeexp; 
    }
