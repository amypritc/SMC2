...
enum exprtype { IDENT = 1, CONSTVAL, STRING, FUNCCALL, ARRAYIDX,
                ...
                DESIGNATED, IDXDES, DOTDES, EPMALLOC};

enum securityflag {PRI=1, PUB};
struct astexpr_
{
  enum exprtype type;
  ...
  int optimal_size;     /* Optimal Variable/Expression Size */
                        /* if size = -1, not specified */
  int optimal_sizeexp;
  ...
  int isptr;      /* if expr has a pointer ?  */
  union { 
...
enum stmttype { JUMP = 1, ITERATION, SELECTION, LABELED, EXPRESSION, 
                DECLARATION, COMPOUND, STATEMENTLIST, FUNCDEF, VERBATIM, SMC, BATCH, OMPSTMT, OX_STMT, SPFREE, 
                BOUNDED_ITERATION};
...
struct aststmt_
{
  ...
  union
  {
    ...
    struct  { aststmt init;
              astexpr cond, incr, bound; } iteration;
    struct  { aststmt init;
              astexpr cond, incr; 
              astexpr bound; 
              int parallel; } bounded_iteration;
    struct  { astexpr cond;
              aststmt elsebody; } selection;
    struct  { astspec spec;
              astdecl decl;       /* dlist is for FuncDef */
              aststmt dlist; } declaration;
    struct  { astexpr variable;
              astexpr party;
              astspec type; 
              astexpr size1;
              astexpr size2;} smcops;
    struct  { astexpr variable;} pfreeops;  
...
#define SFOR         7 
#define SBFOR        8 
#define SIF          9    /* Selections */
#define SSWITCH      10
#define SLABEL       11   /* Labeled */
#define SCASE        12
#define SDEFAULT     13
#define SINPUT       14
#define SOUTPUT      15
...
extern aststmt Iterationstatement(int subtype,
                        aststmt init, astexpr cond, astexpr incr, aststmt body, astexpr bound);
extern aststmt Iterationstatementbounded(int subtype, aststmt init, astexpr cond, 
                        astexpr incr, astexpr bound, int parallel, aststmt body);
...
extern void batch_statement_popAll(batch_statement_stack);
...
#define While(cond,body)         Iterationstatement(SWHILE,NULL,cond,NULL,body,NULL)
#define Do(body,cond)            Iterationstatement(SDO,NULL,cond,NULL,body,NULL)
#define For(init,cond,incr,body) Iterationstatement(SFOR,init,cond,incr,body,NULL)
#define BFor(init,cond,incr,body,bound) Iterationstatement(SFOR,init,cond,incr,body,bound)
...
#define Compound(a)              Statement(COMPOUND,0,a)
     ...
     */
enum clausetype { OCNOCLAUSE = 0, OCNOWAIT, OCIF, OCNUMTHREADS, OCORDERED,
        OCSCHEDULE, OCCOPYIN, OCPRIVATE, OCCOPYPRIVATE,
        OCFIRSTPRIVATE, OCLASTPRIVATE, OCSHARED,
