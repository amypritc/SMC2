/* File generated from [sample-programs/priv-cond-if-neq-flt/if-else-f.c] by PICCO Mon Feb  8 17:41:27 2021
 */
...
int  __original_main(int _argc_ignored, char **_argv_ignored)
{
 ...
 mpz_t* a; 
  ...
  for (int _picco_i = 0; _picco_i < 4; _picco_i++)
        mpz_init(a[_picco_i]);
  __s->smc_set(0.0, a, -1, -1, 32, 9, "float", -1);
  mpz_t* b; 
  b = (mpz_t*)malloc(sizeof(mpz_t) * (4));
  for (int _picco_i = 0; _picco_i < 4; _picco_i++)
        mpz_init(b[_picco_i]);
  __s->smc_set(0.0, b, -1, -1, 32, 9, "float", -1);
...
mpz_t* _picco_priv_float_b_1_then = (mpz_t*)malloc(sizeof(mpz_t) * 4);
for(int _picco_j = 0; _picco_j < 4; _picco_j++){
	mpz_init(_picco_priv_float_b_1_then[_picco_j]);
}
mpz_t* _picco_priv_float_b_1_else = (mpz_t*)malloc(sizeof(mpz_t) * 4);
for(int _picco_j = 0; _picco_j < 4; _picco_j++){
	mpz_init(_picco_priv_float_b_1_then[_picco_j]);
}
__s->smc_set(b, _picco_priv_float_b_1_else, 32, 9, 32, 9, "float", -1);

{
      {
                __s->smc_set(1.2, b, -1, -1, 32, 9, "float", -1);
      }
    }
__s->smc_set(b, _picco_priv_float_b_1_then, 32, 9, 32, 9, "float", -1);
__s->smc_set(_picco_priv_float_b_1_else, b, 32, 9, 32, 9, "float", -1);

    {
      {
                __s->smc_set(2.3, b, -1, -1, 32, 9, "float", -1);
      }
    }
__s->smc_priv_eval(b, _picco_priv_float_b_1_then, _picco_condtmp1, -1);
mpz_clear(_picco_condtmp1);
for(int _picco_j = 0; _picco_j < 4; _picco_j++){
	mpz_clear(_picco_priv_float_b_1_then[_picco_j]);
mpz_clear(_picco_priv_float_b_1_else[_picco_j]);
}
}
