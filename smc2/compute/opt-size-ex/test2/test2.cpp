/* File generated from [sample-programs/test/test2.c] by PICCO Wed Jul 20 10:54:38 2022
 */
...
int  __original_main(int _argc_ignored, char **_argv_ignored)
{
 ...
 mpz_t a;
  mpz_init(a);
 /* size(a) = 12, 12*/
   __s->smc_set(1780, a, -1, 12, "int", -1);
  mpz_t b;
  mpz_init(b);
 /* size(b) = 12, 12*/
   __s->smc_set(3456, b, -1, 12, "int", -1);
  mpz_t c;
  mpz_init(c);
 /* size(c) = 12, 12*/
   __s->smc_set(2345, c, -1, 12, "int", -1);
 ...
 /* size(z) = 32, 0*/
  ...
  __s->smc_add(a, b, _picco_tmp1, 12, 12, 12, "int", -1);
  __s->smc_shr(_picco_tmp1, 1, _picco_tmp1, 12, -1, 12, "int", -1);
  __s->smc_set(_picco_tmp1, x, 12, 32, "int", -1);

  i = 0;
  for ( ;i < 1000; )
  {
    {
      __s->smc_gt(x, c, _picco_tmp1, 32, 12, 1, "int", -1);
