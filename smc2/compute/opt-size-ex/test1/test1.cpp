/* File generated from [sample-programs/test/test1.c] by PICCO Mon Jul 18 22:32:30 2022
 */
...
int  __original_main(int _argc_ignored, char **_argv_ignored)
{
 ...
 mpz_t a;
  mpz_init(a);
 /* size(a) = 3, 3*/
   __s->smc_set(7, a, -1, 3, "int", -1);
  mpz_t b;
  mpz_init(b);
 /* size(b) = 3, 3*/
   __s->smc_set(3, b, -1, 3, "int", -1);
 ...
 /* size(z) = 32, 0*/
  ...
  int i;
    ...
    __s->smc_set(a, x, 3, 32, "int", -1);

    __s->smc_set(b, y, 3, 32, "int", -1);

  i = 0;
  for ( ;i < 1000; )
  {
    {
      __s->smc_gt(x, y, _picco_tmp1, 32, 32, 1, "int", -1);
